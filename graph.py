...
def get_from_api(url, auth, params):
    """Gets all the data for a certain set of parameters from the API"""
    results = []
    ...
    print("{} records retrieved".format(len(results)))
    return results


def localize(results, timezone, datetime_field='upload_date'):
    """Given a list of dicts, each having the key specified in datetime_field containing a time,
    changes the value that key points to to the timezone given in timezone"""
    localized_results = []
